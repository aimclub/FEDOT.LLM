...
class Tee:
    ...
    def write(self, data):
        for stream in self.streams:
            ...
            elif isinstance(stream, multiprocessing.connection.Connection):
                try:
                    stream.send(data)
...
def unsafe_execute(
    ...
    output_dir: Path = None,
    vaults: list[Path] = None,
    pipes: tuple[multiprocessing.Pipe, multiprocessing.Pipe] = None
):
...
@contextlib.contextmanager
def swallow_io(input_stream=None, binary=False, pipes: tuple[multiprocessing.Pipe, multiprocessing.Pipe] = None):
    """
