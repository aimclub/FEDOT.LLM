...
def run_send_message(state: AutoMLAgentState, inference: AIInference):
    if state['solutions'][-1]['code'] is None:
        ...
        message = message.content + "\n\n" + "[[Code]]({code_url}) | [[Pipeline]]({pipeline_url})".format(
            code_url="file://" + str((Path(get_settings().config.result_dir) / "solution.py").resolve()),
            pipeline_url="file://" + str((Path(get_settings().config.result_dir) / "pipeline/pipeline.json").resolve())
        )
        state['messages'] = [HumanMessage(
