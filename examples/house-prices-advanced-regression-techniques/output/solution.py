...
def automl_predict(model, features: pd.DataFrame | pd.Series) -> np.ndarray:
    input_data = InputData.from_numpy(features.to_numpy(), None, task=Task(TaskTypesEnum.regression))
...
DATASET_PATH = Path("/Users/aleksejlapin/Work/STABLE-FedotLLM/examples/house-prices-advanced-regression-techniques/competition") # path for saving and loading dataset(s)
WORKSPACE_PATH = Path("/Users/aleksejlapin/Work/STABLE-FedotLLM/examples/house-prices-advanced-regression-techniques/output")
...
TARGET_COLUMNS = ["SalePrice"] # Replace with your actual target column name(s)
...
def preprocess_data(train_features: pd.DataFrame, test_features: pd.DataFrame):
    ...
    # --- Step 0: Identify feature types (based on training data) ---
    numerical_cols = X_train.select_dtypes(include=["int64", "float64"]).columns.tolist()
    categorical_cols = X_train.select_dtypes(include=["object"]).columns.tolist()

    # --- Step 1: Handle Missing Values (Fit on Train, Transform Train & Test) ---
    # Create an imputer for numerical and categorical features
    ...
    # --- Step 2: Encode Categorical Features (Fit on Train, Transform Train & Test) ---
    ...
    # --- Step 3: Convert to DataFrame for consistency and indexing ---
...
def main():
    ...
    current_problem_type = 'regression'  # Set the problem type
    ...
    if predictions is not None:
        target_cols_in_submission = TARGET_COLUMNS  # Default to TARGET_COLUMNS from training
