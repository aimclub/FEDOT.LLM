...
def test_problem_reflection_success(
    mock_prompts, mock_inference, mock_dataset, initial_state
):
    mock_dataset.dataset_preview.return_value = "Dataset Preview Text"
    mock_dataset.dataset_eda.return_value = "Dataset EDA Text"
    mock_prompts.automl.problem_reflection_prompt.return_value = (
    ...
    mock_prompts.automl.problem_reflection_prompt.assert_called_once_with(
        data_files_and_content="Dataset Preview Text", dataset_eda="Dataset EDA Text"
    )
    mock_inference.query.assert_called_once_with(
        [{"role": "user", "content": "Generated Reflection Prompt"}]
    )
...
@patch("fedotllm.agents.automl.nodes.prompts")
def test_generate_automl_config_success(
    mock_prompts, mock_inference, initial_state, mock_dataset
):
    current_state_dict = dict(initial_state)
    current_state_dict["reflection"] = "Test Reflection"

    mock_prompts.automl.generate_configuration_prompt.return_value = (
    ...
    result_command = generate_automl_config(
        current_state_dict, mock_inference, mock_dataset
    )

    mock_prompts.automl.generate_configuration_prompt.assert_called_once_with(
        reflection="Test Reflection"
    )
    mock_inference.create.assert_called_once_with(
        "Generated Config Prompt", response_model=FedotConfig
    )
    assert isinstance(result_command, Command)
...
def test_select_skeleton_success(
    mock_load_template,
    mock_render_template,
    mock_dataset,
    initial_state,
    automl_nodes_mock_workspace,
):
    ...
    result_command = select_skeleton(
        current_state_dict, mock_dataset, automl_nodes_mock_workspace
    )
...
@patch("fedotllm.agents.automl.nodes.load_template")
def test_select_skeleton_unknown_predict_method(
    mock_load_template_ignored, mock_dataset, initial_state, automl_nodes_mock_workspace
):
...
def test_generate_code_success(
    mock_prompts, mock_extract_code, mock_inference, mock_dataset, initial_state
):
    current_state_dict = dict(initial_state)
    current_state_dict["reflection"] = "Test Reflection"
...
@patch("fedotllm.agents.automl.nodes.render_template")
@patch("fedotllm.agents.automl.nodes.load_template")
def test_insert_templates_success(
    mock_load_template, mock_render_template, initial_state
):
    mock_fedot_config = FedotConfig(
        problem="classification",
        preset="fast_train",
    ...
    current_state_dict = dict(initial_state)
    current_state_dict["raw_code"] = (
        "from automl import train_model, evaluate_model, automl_predict\n# Rest of code"
    )
    current_state_dict["fedot_config"] = mock_fedot_config

    def load_template_side_effect(template_name):
        if template_name == "fedot_train.py":
            return "Raw Template: fedot_train.py"
        if template_name == "fedot_evaluate.py":
            return "Raw Template: fedot_evaluate.py"
        if template_name == "fedot_predict.py":
            return "Raw Template: fedot_predict.py"
        return None
...
@patch("fedotllm.agents.automl.nodes.render_template")
@patch("fedotllm.agents.automl.nodes.load_template")
def test_insert_templates_exception_handling(
    mock_load_template, mock_render_template, initial_state
):
    mock_fedot_config = FedotConfig(
        problem="classification",
...
def test_evaluate_success(
    ...
    mock_observation = Observation(
        stdout="Run successful", stderr="", error=False, msg="OK"
    )
    mock_execute_code.return_value = mock_observation

    result_command = evaluate(current_state_dict, automl_nodes_mock_workspace)

    mock_generate_code_file.assert_called_once_with(
        "valid python code", automl_nodes_mock_workspace
    )
...
def test_evaluate_execution_error(
    ...
    mock_observation = Observation(
        stdout="", stderr="Syntax Error", error=True, msg="Error"
    )
    mock_execute_code.return_value = mock_observation
    ...
    mock_generate_code_file.assert_called_once_with(
        "buggy python code", automl_nodes_mock_workspace
    )
    mock_execute_code.assert_called_once_with(path_to_run_code=mock_code_file_path)
    assert isinstance(result_command, Command)
    assert result_command.update == {"observation": mock_observation}
...
def test_if_bug_is_true(mock_get_settings, initial_state):
    ...
    current_state_dict["observation"] = Observation(
        error=True, msg="Error occurred", stdout="", stderr="Traceback..."
    )
...
def test_if_bug_is_false_no_error(mock_get_settings, initial_state):
    ...
    current_state_dict = dict(initial_state)
    current_state_dict["observation"] = Observation(
        error=False, msg="OK", stdout="Success", stderr=""
    )
    current_state_dict["fix_attempts"] = 1
...
def test_fix_solution_success(
    ...
    current_state_dict["observation"] = Observation(
        stdout="out", stderr="err", error=True, msg="Error details"
    )
    ...
    assert isinstance(result_command, Command)
    assert result_command.update == {
        "raw_code": "Extracted Fixed Code",
        "fix_attempts": 1,
    }
...
def test_extract_metrics_success(
    mock_fedot_api, mock_graph_structure, automl_nodes_mock_workspace, initial_state
):
    current_state_dict = dict(initial_state)
    current_state_dict["observation"] = Observation(
        stdout="Model metrics: {'accuracy': 0.95}", error=False, msg="OK"
    )
    ...
    assert (
        hasattr(load_call_args[0], "_spec_class")
        or str(load_call_args[0].__class__) == "<class 'unittest.mock.MagicMock'>"
    )
...
def test_extract_metrics_no_metrics_found(initial_state):
    current_state_dict = dict(initial_state)
    current_state_dict["observation"] = Observation(
        stdout="No metrics here", error=False, msg="OK"
    )
...
def test_extract_metrics_pipeline_not_found(initial_state):
    current_state_dict = dict(initial_state)
    current_state_dict["observation"] = Observation(
        stdout="Model metrics: {'accuracy': 0.9}", error=False, msg="OK"
    )
...
@patch("fedotllm.agents.automl.nodes.Fedot")
def test_extract_metrics_exception_handling(
    mock_fedot_api, automl_nodes_mock_workspace, initial_state
):
    current_state_dict = dict(initial_state)
    current_state_dict["observation"] = Observation(
        stdout="Model metrics: {'accuracy': 0.9}", error=False, msg="OK"
...
def test_run_tests_all_pass(
    ...
    updated_state = run_tests(
        current_state_dict, automl_nodes_mock_workspace, mock_inference
    )
...
@patch("fedotllm.agents.automl.nodes.Fedot")
@patch("fedotllm.agents.automl.nodes.pd.read_csv")
def test_run_tests_metrics_fail(
    mock_pd_read_csv,
    mock_fedot_api_rt,
    mock_graph_structure_rt,
    mock_inference,
    ...
    submission_file_mock.exists.return_value = True
    submission_file_mock.suffix = ".csv"
    automl_nodes_mock_workspace.__truediv__.return_value = (
        submission_file_mock  # Make sure / operator returns this mock
    )
    mock_pd_read_csv.side_effect = [
        pd.DataFrame({"col1": [1]}),
        pd.DataFrame({"col1": [1]}),
...
@patch("fedotllm.agents.automl.nodes.Fedot")
@patch("fedotllm.agents.automl.nodes.pd.read_csv")
def test_run_tests_submission_file_not_found(
    mock_pd_read_csv,
    mock_fedot_api_rt,
    mock_graph_structure_rt,
    mock_inference,
    ...
    automl_nodes_mock_workspace.__truediv__.side_effect = (
        truediv_side_effect_sub_not_found
    )
...
@patch("fedotllm.agents.automl.nodes.Fedot")
@patch("fedotllm.agents.automl.nodes.pd.read_csv")
def test_run_tests_submission_format_fail_llm(
    mock_pd_read_csv,
    mock_fedot_api_rt,
    mock_graph_structure_rt,
    mock_inference,
