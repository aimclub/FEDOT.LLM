...
from fedotllm.data import Dataset, Split, load_pd, missing_values
...
def test_load_pd_arff(
    mock_pd_dataframe_constructor, mock_fedot_loadarff, mock_path, mock_df
):
...
class TestDataset:
    ...
    def test_get_train_split_max_cols(self):
        ...
        s2 = Split(
            "s2.csv", pd.DataFrame({"x": [1], "y": [2], "z": [3]})
        )  # 3 cols (train)
    ...
    def test_get_train_split_max_rows_tie_break(self):
        s1 = Split(
            "s1.csv", pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
        )  # 3 rows, 2 cols
        s2 = Split("s2.csv", pd.DataFrame({"x": [1], "y": [2]}))  # 1 row, 2 cols
        s_train = Split(
            "s_train.csv", pd.DataFrame({"c": [1, 2, 3, 4], "d": [5, 6, 7, 8]})
        )  # 4 rows, 2 cols (train)
    ...
    def test_dataset_eda_less_than_10_cols(
        self, mock_missing_vals_func, mock_get_train, mock_df
    ):
        ...
        mock_missing_vals_func.return_value = pd.DataFrame(
            {"Missing": [1], "Percent": [50.0]}, index=["col1"]
        )
    ...
    def test_dataset_preview_train_gt_10_cols(self, mock_get_train):
        ...
        with (
            patch.object(train_df, "sample") as mock_train_sample,
            patch.object(other_df, "sample") as mock_other_sample,
        ):
            mock_train_sample.return_value.to_markdown.return_value = (
                "train_markdown_sample"
            )
    ...
    def test_dataset_preview_train_lte_10_cols(self, mock_get_train):
        ...
        with (
            ...
            mock_train_sample.return_value.to_markdown.return_value = (
                "train_markdown_sample_small"
            )
            mock_other_sample.return_value.to_markdown.return_value = (
                "other_markdown_sample_small"
            )
