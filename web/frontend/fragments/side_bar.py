...
def set_model(form_grid: GridDeltaGenerator):
    ...
    params = {'model': name, 'temperature': 0}
...
def set_openai_model(form_grid: GridDeltaGenerator, params: dict):
    ...
    try:
        ...
        st.toast(f"Successfully set up a model:\n {st.session_state.model_name}", icon="✅")
    ...
    match provider:
        case 'Ollama':
            grid.text_input("model", placeholder="llama3.1",
                            key="model_name_input", disabled=bool(st.session_state.model))
        case 'OpenAI':
            grid.text_input("model", placeholder="gpt-3.5-turbo",
                            key="model_name_input", disabled=bool(st.session_state.model))
            grid.text_input("token", placeholder="Your openai token",
                            key="openai_api_key", disabled=bool(st.session_state.model),
                            type='password')
            grid.text_input("base url", placeholder="Optional base url", key="base_url",
...
def init_model():
    with st.container(border=True):
        ...
        if not st.session_state.model:
            if not st.session_state.model:
                on_provider = st.selectbox("Select provider", options=['Ollama', 'OpenAI'], key='model_provider_input',
                                           placeholder='Select provider')
                form_grid = grid(1, 1, 1, 1, 1, vertical_align='bottom')
                if on_provider:
                    on_provider_selected(form_grid)
                submit = st.button(label="Submit", use_container_width=True, disabled=bool(st.session_state.model))
                if submit:
                    set_model(form_grid)
        else:
