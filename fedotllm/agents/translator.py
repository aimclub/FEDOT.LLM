...
class TranslatorAgent:
    ...
    def _extract_code_blocks(self, text: str) -> tuple[str, dict[str, str]]:
        ...
        if idx > 0:
            logger.debug(f"_extract_code_blocks: Found and replaced {idx} code blocks. Placeholders: {list(code_blocks_map.keys())}")
        else:
    ...
    def _reinsert_code_blocks(self, text: str, code_blocks_map: dict[str, str]) -> str:
        ...
        logger.debug(f"_reinsert_code_blocks: Attempting to reinsert {len(code_blocks_map)} code block(s) from map: {list(code_blocks_map.keys())}")
        reinserted_count = 0
        not_found_placeholders = []
        for placeholder, original_code_block in code_blocks_map.items():
            escaped_placeholder = re.escape(placeholder)
            text_after_sub, num_replacements = re.subn(escaped_placeholder, lambda m: original_code_block, text, count=1)
            logger.debug(f"_reinsert_code_blocks: Searching for placeholder: '{placeholder}'. Found and replaced: {num_replacements > 0} (Count: {num_replacements})")
            if num_replacements > 0:
                reinserted_count += 1
                text = text_after_sub
            else:
                logger.warning(f"_reinsert_code_blocks: Placeholder '{placeholder}' not found in the translated text for re-insertion.")
                not_found_placeholders.append(placeholder)
        if reinserted_count == len(code_blocks_map) and not not_found_placeholders:
            logger.debug(f"_reinsert_code_blocks: Successfully reinserted all {reinserted_count} code blocks.")
        else:
            logger.warning(
    ...
    def _translate_text(self, text: str, target_language: str, source_language: str = "auto") -> str:
        logger.info(f"TranslatorAgent._translate_text: Attempting translation from '{source_language}' to '{target_language}' using self.inference.query.")
        logger.debug(f"Original text for _translate_text (first 200 chars): '{text[:200]}...'")

        processed_text, code_blocks_map = self._extract_code_blocks(text)
        if text != processed_text:
            logger.debug(f"Text after code block extraction (first 200 chars): '{processed_text[:200]}...'")
        ...
        try:
            logger.debug(f"TranslatorAgent: Sending prompt to self.inference.query (from '{source_language}' to '{target_language}'):\n{prompt}")
            # Using self.inference.query, expecting a direct string response
            response_text = self.inference.query(prompt) 
            logger.debug(f"TranslatorAgent: Received response from self.inference.query. Type: {type(response_text)}. Content (first 200 chars): '{str(response_text)[:200]}...'")

            if response_text and isinstance(response_text, str):
                translated_text_with_placeholders = response_text
                if translated_text_with_placeholders != processed_text:
                    logger.info(f"Successfully translated text from {source_language} to {target_language} using self.inference.query.")
                else:
                    logger.info(f"Text from {source_language} to {target_language} may not have been translated by LLM (output is same as input to LLM, or LLM returned empty).")
            elif not response_text:
                 logger.warning("self.inference.query returned empty or None response.")
            else: # Response is not a string and not None/empty
                logger.warning(f"self.inference.query returned unexpected type: {type(response_text)}. Expected str. Using original text as fallback.")

        except Exception as e:
            logger.error(f"Error during translation using self.inference.query from '{source_language}' to '{target_language}': {e}", exc_info=True)
            # Fallback to processed_text (placeholders intact, no translation) is already default

        logger.debug(f"TranslatorAgent._translate_text: Text before re-inserting code blocks (placeholders should be visible):\n{translated_text_with_placeholders}")
        final_translated_text = self._reinsert_code_blocks(translated_text_with_placeholders, code_blocks_map)
        logger.debug(f"Final translated text after reinserting code blocks (target: {target_language}, first 200 chars): '{final_translated_text[:200]}...'")
        return final_translated_text

    def translate_input_to_english(self, message: str) -> str:
        logger.info(f"TranslatorAgent: Received input message for translation to English (first 200 chars): '{message[:200]}...'")
        try:
            self.source_language = detect(message)
        except LangDetectException as e:
            self.source_language = 'en'
            logger.warning(f"Language detection failed for input message (defaulting to 'en'): {e}", exc_info=True)
        
        logger.info(f"TranslatorAgent: Source language for input set to: {self.source_language}")

        if self.source_language != 'en':
            logger.info(f"Translating input from {self.source_language} to English using self.inference.query.")
    ...
    def translate_output_to_source_language(self, message: str) -> str:
        logger.info(f"TranslatorAgent: Attempting output translation. Current source_language_for_session: {self.source_language}")
        ...
        if self.source_language and self.source_language != 'en':
            logger.info(f"Translating output from English to {self.source_language} using self.inference.query.")
            logger.info(f"English message for output translation (first 200 chars): '{message[:200]}...'")
            return self._translate_text(message, target_language=self.source_language, source_language='en')
        elif not self.source_language:
            logger.warning("Cannot translate output: source_language_for_session not set (input might not have been processed or detection failed).")
            return message
        else: 
            logger.info(f"Output translation not needed (source session language was '{self.source_language}'). Returning original English message.")
            logger.debug(f"Original English message for output (first 200 chars): '{message[:200]}...'")
            return message
