#!/usr/bin/env python3
"""
MLZero Bio Competition - Molecular Biological Response Prediction Script using AutoGluon
This script predicts binary molecular biological responses using AutoGluon's TabularPredictor
Binary classification task with ROC AUC evaluation metric
"""
...
# Configuration
DATA_DIR = "mlzero/bio/competition"
...
print("=" * 80)
print("MLZero Bio Competition - Molecular Response Prediction using AutoGluon")
print("=" * 80)
...
print(f"Positive response rate: {train_data[TARGET_COLUMN].mean()*100:.2f}%")
...
print(f"Number of features (D1-D1776): {len(train_features.columns) - 1}")  # -1 for target
...
predictor.fit(
    ...
    presets='medium_quality',  # Use best quality preset for better performance
...
print(f"Validation performance (ROC AUC): {val_performance}")
...
predictions_proba = predictor.predict_proba(test_features)

# For binary classification, get probability of positive class (target=1)
if isinstance(predictions_proba, pd.DataFrame):
    predictions = predictions_proba[1]  # Probability of class 1 (response present)
else:
...
print(f"Min probability: {submission['target'].min():.4f}")
print(f"Max probability: {submission['target'].max():.4f}")
