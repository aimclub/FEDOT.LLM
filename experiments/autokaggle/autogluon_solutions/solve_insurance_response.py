#!/usr/bin/env python3
"""
Kaggle Playground Series S4E7 - Insurance Response Prediction Script using AutoGluon
This script predicts which customers respond positively to an automobile insurance offer using AutoGluon's TabularPredictor
"""
...
# Configuration
DATA_DIR = "autokaggle/playground-series-s4e7"
...
print("=" * 80)
print("Playground Series S4E7 - Insurance Response Prediction using AutoGluon")
print("=" * 80)
...
if isinstance(predictions_proba, pd.DataFrame):
    predictions = predictions_proba[1]  # Probability of class 1 (positive response)
else:
...
print(f"Mean probability: {submission['Response'].mean():.4f}")
print(f"Min probability: {submission['Response'].min():.4f}")
print(f"Max probability: {submission['Response'].max():.4f}")
