#!/usr/bin/env python3
"""
Kaggle Bank Churn Prediction Script using AutoGluon
This script predicts customer churn using AutoGluon's TabularPredictor
"""
...
# Configuration
DATA_DIR = "autokaggle/bank-churn"
...
print("=" * 80)
print("Bank Churn Prediction using AutoGluon")
print("=" * 80)
...
# Identify columns to drop (id, CustomerId, Surname are not useful for prediction)
...
print(f"Features used for training: {list(train_features.columns)}")
...
predictor = TabularPredictor(
    label=TARGET_COLUMN,
    eval_metric='roc_auc',  # ROC AUC is the evaluation metric for this competition
    problem_type='binary',
    verbosity=2
)
...
predictor.fit(
    train_data=train_df,
    tuning_data=val_df,
    time_limit=60,  # 10 minutes time limit
...
# For binary classification, AutoGluon returns probabilities for both classes
# We need the probability of the positive class (Exited=1)
if isinstance(predictions, pd.DataFrame):
    predictions_proba = predictions[1]  # Probability of class 1
else:
